...
export class CheckResult<T extends CheckType> {
    isAlive: boolean;
    type: T;
    receivedData: T extends CheckType.PING ? IPingReceivedData : T extends CheckType.SSL ? ISslReceivedData : T extends CheckType.HTTP ? IHttpReceivedData : T extends CheckType.HEALTHCHECK ? IHealthcheckReceivedData : never;
}
...
export class AvailableCheckerService {
    ...
    async makeResolution(checkResult: CheckResults) {
        ...
        if (aliveChecks >= 3) {
            ...
            checkResolution.checkMethods = checkResult.checkMethods.filter((method: string) => {
                switch (method) {
                    case 'health':
